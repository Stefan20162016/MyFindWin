...
grepCPPI is slower than grep-mode if files are in page-cache, faster on new
...
uint64_t hits{ 0 };
// filename of this executable: 'mfgo' mean myfindgrep with O_DIRECT, everything else page-cached
std::string exe_name;

//std::atomic<long long> sum_file_size{0}; // if you want disk usage stats like `du`
...
public:
    Worker(int n, std::string s) : worker_id(n), start_with_path(s) {
        if (worker_id == -42) // temp worker for startup
            ...
            //buffer = (char*)memalign(4096, buffer_size); // memalign for O_DIRECT i.e. no page-cache
            int x;
    }
    ~Worker() {
        if (worker_id == -42) {
    ...
    void operator()() {
        try {
            ...
            std::cout << worker_id << ": WORKER WORKER string: " << start_with_path << std::endl; }

            //buffer = (char*)malloc(buffer_size);
            //buffer = (char*)memalign(4096 * 2, buffer_size); // memalign for O_DIRECT i.e. no page-cache
            if (start_with_path != "") {
...
#define RETRY_COUNT 0 // a few or a few dozen works equally well; deprecated with atomics
        ...
        int dont_wait_forever = 64; // to eliminate endless waiting at the 'end of the tree':few dirs left
...
#define NR_OF_ROUNDS 1 // a few or just one round works equally well
        ...
        if (retry_count-- > 0) {
            ...
            for (int i = 0; i < NR_ATOMICS; i++) {
                if (atomic_running_threads[i] > 0) {
                    ...
                    if (dont_wait_forever-- > 0) { // protection for way too many threads (> remaining directories)
                        goto check_again;
                    }
                    else {
                        break;
    ...
    */
    ...
    of a long line.  P[LIM] should be EOL.  code from grep.c */
    void zap_nuls(char* p, char* lim, char eol) {
        if (eol)
            while (true) {
                *lim = '\0';
                p += strlen(p);
    ...
    int find_or_grep(const char* filename) {
        ...
        else if (search_mode == "grepCPPI" || search_mode == "grep2") { // no binary
            std::ifstream src(filename);
            if (!src.good()) {
                std::cout << "error with filename: " << filename << std::endl;
                //throw std::ios::failure("src is no good: " ); don't throw or else atomic won't be decremented
...
private:
    ...
    int do_dir_walking(const std::string& sInputPath) {
        ...
        try {
            ...
            for (; dir_iter != end; dir_iter.increment(ec))
            {
                ...
                if ((std::filesystem::is_directory(path, ec) || std::filesystem::is_regular_file(path, ec)) || std::filesystem::is_symlink(path, ec)) {
                    ...
                    /*
                        ...
                        int hash = -1;
                        hash = find_or_grep(path_entry.c_str());
            ...
            std::cout << ex.code().message() << ex.code() << ex.what() << std::endl;
